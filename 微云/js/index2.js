(function(){
	...
	on(rename,'click',function(){
		...
		if(!whichSelect.length){
			tips('err','请选中要重命名的文件夹')
		}
		if(whichSelect.length > 1){
			tips('err','不能同时重命名多个文件')
		}
		if(whichSelect.length === 1){
	...
	on(remove,'click',function(){
		...
		var spanId = span.dataset.id;
		var whichSelect = select();
//		console.log(whichSelect,'whichSelect')
		if(!whichSelect.length){			
			tips('err','请选中文件夹')
		}else{
			...
			function moveFn(ev){
				...
				if(target.nodeName === 'SPAN'){
					...
					warn.innerHTML = '';
					var divs = dirTree.getElementsByTagName("div");
					for( var i = 0; i < divs.length; i++ ){
					...
					function fn(){	
						for( var i = 0; i < whichSelect.length; i++ ){
							var id = whichSelect[i].parentNode.dataset.id;
							var item = getDataById(id);
							//判断是否重名；
							if(!reNameState(selId,item.title)){
								whichSelect[i].parentNode.remove();
								item.pid = selId;
							}else{
						...
						var childs = getChildsById(currentId);
						checkedAll.classList.remove('checked');	
						var filesHtml = creatFiles(currentId);
						if(filesHtml === ''){
							fEmpty.style.display = 'block';
						}else{
							fEmpty.style.display = 'none';							
